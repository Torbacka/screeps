...
function withdraw(creep, roomName) {
    if (creep.memory.collect && creep.store.getFreeCapacity() === 0) {
        ...
        const structuresWithEnergy = creep.pos.findClosestByRange(123, {
            filter: structure => {
                return structure.store && structure.store.getUsedCapacity() > 0;
            }
        });

       if (structuresWithEnergy) {
           Object.keys(structuresWithEnergy.store).forEach(resourceType => {
               if (creep.withdraw(structuresWithEnergy, resourceType) === ERR_NOT_IN_RANGE) {
           ...
           let tombstones = creep.room.find(FIND_DROPPED_RESOURCES);
           if (tombstones.length > 0) {

               Object.keys(tombstones[0]).forEach(resourceType => {
                   if (creep.pickup(tombstones[0]) === ERR_NOT_IN_RANGE) {
                       creep.moveTo(tombstones[0]);
                   }
               });
