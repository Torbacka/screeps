module.exports = function () {
    if (Game.time % 100 === 0) {
        ...
        const price = 18; // Desired price per unit
        const amount = 50000; // Desired amount to buy
        ...
        energySellOrders = energySellOrders.map(order => {
            ...
            order.totalCostPerUnit = (order.price) / ((order.amount - transferCost) / (order.amount));
        ...
        const existingBuyOrders = Game.market.getAllOrders(order =>
        ...
        if (existingBuyOrders.length === 0) {
            ...
            if (result === OK) {
                ...
                console.log(`Failed to create buy order. Error code: ${result}`);
            }
        }
    }
